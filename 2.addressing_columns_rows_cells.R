...
air_temp2 <- read.table("G:/EASY_R/air_temp_edited.csv", header=TRUE, sep=",")

#2: Check the data type of the "time" column
str(air_temp2$time)

#3: If the time column is not a POSIXct object, then convert it
air_temp2$time<-as.POSIXct(air_temp2$time, "%Y-%m-%d %H:%M:%S", tz="")
...
#Izpiši vse vrednosti, ki so pred datumom 2015-01-01 
which(air_temp2$time < as.POSIXct('2015-01-01 00:00:00', format ='%Y-%m-%d %H:%M:%S', tz=''))

#subsetting our dataframe getting some rows only based on values of a column
air_temp_bef_2015 <- air_temp2[which(air_temp2$time < as.POSIXct('2015-01-01 00:00:00', format ='%Y-%m-%d %H:%M:%S', tz='')),] #would return all rows before 2015
...
write.table(air_temp_2016,"G:/EASY_R/air_temp_sub_row_2016.csv", sep=",", 
...
v<-c(1,6,7,10)
v2<-c("Gregor","Erika","Koffi")
length(v2)
length(air_temp2$time [which(air_temp2$year==2015)])
#izpiši vse vrednosti v tabeli air_temp2 in sicer vse vrstice v stolpcih 1, 2 in 3
air_temp2[c(1,4,5), c(1:3)]

air_temp_some_cols <- air_temp2[, c("time","hour")]
